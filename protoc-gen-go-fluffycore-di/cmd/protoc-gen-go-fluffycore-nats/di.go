...
func (s *methodGenContext) generateClientMethodShim() {
	...
	*/
	...
	g.P("func (s *", internalClientName, ") ", s.grpcClientMethodSignature(), "{")
	g.P("\tresponse := &", g.QualifiedGoIdent(method.Output.GoIdent), "{}")
...
func (s *methodGenContext) generateNATSMethodGRPCGateway() {
	...
	g.P("				},")
	g.P("				func(r *", g.QualifiedGoIdent(method.Input.GoIdent), ") error {")
	g.P("					return ", protoPackage.Ident("Unmarshal"), "(req.Data(), r)")
	g.P("				},")
	/*
	...
	*/
	...
	g.P("				err := ", protoPackage.Ident("Unmarshal"), "(req.Data(), request)")
	g.P("				if err != nil {")
	g.P("					return nil, err")
	g.P("				}")
	g.P("				return request,nil")
	g.P("			},")
	g.P("\t\tfunc(pm ", protoreflectPackage.Ident("ProtoMessage"), ",req *", g.QualifiedGoIdent(method.Input.GoIdent), ") error{")
	g.P("				pj, err := ", protoPackage.Ident("Marshal"), "(pm)")
	g.P("				if err != nil {")
	g.P("					return err")
	g.P("				}")
	g.P("				return ", protoPackage.Ident("Unmarshal"), "(pj, req)")
	g.P("			},")
	g.P("				func(ctx ", contextPackage.Ident("Context"), ", request *", g.QualifiedGoIdent(method.Input.GoIdent), ") (*", g.QualifiedGoIdent(method.Output.GoIdent), ", error) {")
	...
	g.P("\t\t\t\"request_schema\": ", fluffyCoreUtilsPackage.Ident("SchemaFor"), "(&", g.QualifiedGoIdent(method.Input.GoIdent), "{}),")
	g.P("\t\t\t\"response_schema\": ", fluffyCoreUtilsPackage.Ident("SchemaFor"), "(&", g.QualifiedGoIdent(method.Output.GoIdent), "{}),")
	g.P("		}),")
